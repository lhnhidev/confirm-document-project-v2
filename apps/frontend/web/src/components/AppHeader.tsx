...
import { IconSchool, IconLogout, IconUserCheck, IconChevronDown, IconBell } from "@tabler/icons-react"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Menu shadow="md" width={240} position="bottom-end">
            ...
            <Menu.Dropdown className="bg-slate-900 border-slate-800 text-slate-200">
              <Menu.Label className="text-slate-400">Tài khoản cán bộ</Menu.Label>
              <Menu.Item
                leftSection={<IconUserCheck size={16} className="text-blue-400" />}
                className="hover:bg-slate-800"
              >
                <div>
                  <Text size="xs" fw={600}>{currentUser.fullName}</Text>

                  <Text size="xs" c="dimmed">{currentUser.email}</Text>
                  <Text size="xs" c="dimmed">Môn: {currentUser.major}</Text>
                </div>
              </Menu.Item>

              <Menu.Divider className="border-slate-800" />
